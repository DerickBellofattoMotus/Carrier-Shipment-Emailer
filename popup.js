...
function renderSummary(raw) {
  ...
  const locLines = shipLocations.map((loc) => {
    ...
    const parts = [t, city, state, dtFormatted, apptType ? `ApptType: ${apptType}` : ''].filter(Boolean);
    return parts.join(' - ');
  });
